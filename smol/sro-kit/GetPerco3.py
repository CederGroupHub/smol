...
if __name__ == "__main__":
    ...
    for Root, Dir, Files in os.walk('%s/%s'%(Prefix,args.Conf)):
        if 'POSCAR' in Files and 'TMM' not in Root and 'xLi' not in Root: CalcDirs.append(Root);
        elif 'POSCAR.gz' in Files and 'TMM' not in Root and 'xLi' not in Root: CalcDirs.append(Root);
        elif 'POSCAR.bz2' in Files and 'TMM' not in Root and 'xLi' not in Root: CalcDirs.append(Root);
   
    print('Evaluating the following directories {}'.format(CalcDirs));
