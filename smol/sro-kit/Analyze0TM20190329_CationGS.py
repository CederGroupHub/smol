...
if __name__ == "__main__":
    ...
    for Root, Dir, Files in os.walk(args.WorkingDir):
        if (('POSCAR' in Files) or ('POSCAR.gz' in Files) or ('POSCAR.bz2' in Files)) \
                and 'TMM' not in Root and 'xLi120' not in Root and 'CationGS' in Root:
                    CalcDirs.append(Root);
    #print(calcdirs,len(calcdirs)); exit();
    ...
    with open('{}/TMInfo_CationGS.dat'.format(args.WorkingDir),'w') as Fid: Fid.write(str(InfoLsts));
