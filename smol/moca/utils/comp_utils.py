...
def get_n_links(comp_stat, flip_table):
    ...
    Args:
        comp_stat:
            a list of lists, same as the return value of
            comp_utils.occu_to_compstat, record the number of occupying
            species on each sublattice.
        flip_table:
    ...
    for op_id, mult_flip in enumerate(flip_table):
        ...
        for n in n_to_flip_on_sl:
            if n != 0:
                raise ValueError("Number of species on both sides of \
                                  mult_flip can not match!")
