...
def run_T(ecis, cluster_supercell, occu, T, n_loops, ind_groups, n_rand=0,
          check_unique=True, sample_energies=0):
    """
    Run a production run at a single temperature

    :param eg: ECI Generator Object
    :param cluster_supercell:  Cluster Supercell Object
    ...
    :return: current occupation, minimum energy occupation observed,
             minimum energy, list of random structures,list of sampled energies
             OR
             current occupation, minimum energy occupation observed,
             minimum energy, list of sampled energies
    """
    ...
    if len(ind_groups) == 0:
        raise ValueError("MC: all index groups are empty,"
                         "so there are no valid flips!")
    ...
    for i, loop in enumerate(range(n_loops)):
        ...
        d_corr, new_occu = cluster_supercell.delta_corr(flips, occu,
                                                        debug=False)
        ...
        # Save random structures that have different energies
        # that existing saved structures
...
def simulated_anneal(ecis, cluster_supercell, occu, ind_groups, n_loops,
                     init_T, final_T, n_steps, return_E=False):
    ...
    for i in range(n_steps):
        T = T - T_step
        occu, min_occu_step, min_e, rand_occu = run_T(ecis, cluster_supercell,
                                                      deepcopy(min_occu), T,
                                                      n_loops, ind_groups,
                                                      n_rand=0,
                                                      check_unique=True)
