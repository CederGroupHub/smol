...
class WangLandau(MCKernel):
    ...
    def single_step(self, occupancy, walker=0):
        ...
        if self._window[0] is not None and new_energy < self._window[0]:  # reject
            self.trace.accepted = np.array(False)
        elif self._window[1] is not None and new_energy > self._window[0]:  # reject
            self.trace.accepted = np.array(False)
        else:
            new_bin_num = self._get_bin(new_energy)
            new_state = self._aux_states[new_bin_num][walker]
            self.trace.accepted = np.array(
                state[0] - new_state[0] >= log(self._rng.random())
            )
