...
def test_int_container(IntArrayContainer, dim, rng):
    array_list = [rng.integers(1, 5, size=rng.integers(1, 5)) for _ in range(10)]
    container = IntArrayContainer(array_list)
    assert len(container) == 10

    # test setting the same sized list (no memory reallocation)
    new_array_list = [rng.integers(1, 5, size=dim) for _ in range(10)]
