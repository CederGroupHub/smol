...
class TestvsPyabinitio(unittest.TestCase):
    ...
    def _test_dataset(self, dataset):
        ...
        self.assertTrue(np.allclose(sw.feature_matrix, dataset["feature_matrix"]))
    ...
    def test_canonical_montecarlo(self):
        ...
        test_struct.replace_species(
            {"Li+": {"Li+": 2}, "Ni3+": {"Ni3+": 2}, "Ni4+": {"Ni4+": 0}}
        )
        Li_c, Ni3_c = 0.8, 0.4
        test_struct.replace_species(
            {"Li+": {"Li+": Li_c}, "Ni3+": {"Ni3+": Ni3_c, "Ni4+": 1 - Ni3_c}}
        )
        ...
        matrix = np.array([[1, 1, 1], [1, -3, 1], [1, 1, -3]])
        ...
        self.assertAlmostEqual(
            lno_gc_run_10000["min_energy"] / n_atoms,
