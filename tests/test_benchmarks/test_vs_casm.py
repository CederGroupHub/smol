...
class TestvsCASM(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        data_dir = Path(__file__).absolute().parent/'..'/'data'/'benchmark_data'/'casm_mgcro'
    ...
    def test_clusterexpansion(self):
        ...
        est.fit(self.sw.feature_matrix,
                self.sw.get_property_vector('mixing_energy'))
        ecis = est.coef_
        self.assertAlmostEqual(self.fit_data['ecis'][0], ecis[0],
                               places=1)
        self.assertAlmostEqual(sum(np.array(self.fit_data['ecis'])**2),
                               sum(ecis**2), places=2)
