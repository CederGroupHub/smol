...
from smol.moca import MuSemiGrandEnsemble, Sampler, ClusterExpansionProcessor
...
class TestCEvicet(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.coefs = np.linalg.lstsq(cls.sw.feature_matrix,
    ...
    def test_sgc_montecarlo(self):
        ...
        for temp, dmu_dict in icet_sgc_run_10000its.items():
            icet_avg = np.array(
                [d['average_energy'] for _, d in dmu_dict.items()])
            avg = []
            ...
            for dmu, data in dmu_dict.items():
                ...
                self.assertAlmostEqual(data['acceptance_ratio'],
