...
with open(
    os.path.join(DATA_DIR, "benchmark_data/icet_aupd", "aupd_prim.json"), "r"
) as f:
    aupt_prim = Structure.from_dict(json.load(f))

with open(
    os.path.join(DATA_DIR, "benchmark_data/icet_aupd", "aupd_linreg_eci.json"), "r"
) as f:
    icet_eci = np.array(json.load(f))

with open(
    os.path.join(DATA_DIR, "benchmark_data/icet_aupd", "aupd_predictions.json"), "r"
) as f:
    icet_predictions = np.array(json.load(f))

with open(
    os.path.join(DATA_DIR, "benchmark_data/icet_aupd", "fit_structs.json"), "r"
) as f:
...
with open(
    os.path.join(DATA_DIR, "benchmark_data/icet_aupd", "test_structs.json"), "r"
) as f:
