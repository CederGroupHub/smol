...
def test_add_data(structure_wrangler):
    ...
    structure_wrangler.add_data(
        struct, {'energy': energy}, weights={'random': 3.0})

    assert all(w == 2.0 for w in structure_wrangler.get_weights('random')[:-1])
    assert len(structure_wrangler.get_weights('random')) == structure_wrangler.num_structures
    ...
    structure_wrangler.add_properties(
        'normalized_energy', structure_wrangler.get_property_vector('energy', normalize=True))
    ...
    for item in items:
        structure_wrangler.add_data(
            item['structure'], item['properties'],
...
def test_append_data_items(structure_wrangler):
    ...
    for item in items:
        data_items.append(
            structure_wrangler.process_structure(item['structure'],
                                                 item['properties'],
                                                 supercell_matrix=item['scmatrix'],
                                                 site_mapping=item['mapping'])
        )
...
def test_data_indices(structure_wrangler):
    ...
    assert all(
        key in structure_wrangler.available_indices for key in ['test', 'train']
    )
    with pytest.raises(ValueError):
        structure_wrangler.add_data_indices(
            'bla', [structure_wrangler.num_structures, ]
        )
...
def test_properties(structure_wrangler):
    assert structure_wrangler.feature_matrix.shape == (structure_wrangler.num_structures, structure_wrangler.cluster_subspace.num_corr_functions)
    assert len(structure_wrangler.occupancy_strings) == structure_wrangler.num_structures
    num_prim_sites = len(structure_wrangler.cluster_subspace.structure)
    for struct, occu, size in zip(structure_wrangler.structures,
...
def test_matrix_properties(structure_wrangler):
    ...
    assert (structure_wrangler.get_feature_matrix_rank(rows, cols) >=
            structure_wrangler.get_feature_matrix_rank(cols=cols[:-3]))


def test_get_orbit_rank(structure_wrangler):
    for _ in range(10):
        oid = random.choice(range(1, len(structure_wrangler.cluster_subspace.orbits) + 1))
...
def test_get_duplicate_corr_inds(structure_wrangler):
    ...
    with pytest.warns(UserWarning):
        structure_wrangler.add_data(
            dup_item["structure"], dup_item["properties"])

    assert [ind, structure_wrangler.num_structures - 1] in structure_wrangler.get_duplicate_corr_indices()
...
def test_get_matching_corr_duplicate_inds(structure_wrangler):
    ...
    assert all(
        i in matches for matches, expected in
        zip(structure_wrangler.get_matching_corr_duplicate_indices(),
...
def test_msonable(structure_wrangler):
    ...
    assert all(
        [s1 == s2 for s1, s2 in zip(sw.refined_structures, structure_wrangler.refined_structures)]
    )
    npt.assert_array_equal(
        sw.feature_matrix, structure_wrangler.feature_matrix
    )
    assert sw.metadata == structure_wrangler.metadata
    assert all(
        i1['mapping'] == i1['mapping'] for i1, i2 in
        zip(structure_wrangler._items, sw._items)
    )
    assert all(
        np.array_equal(m1, m2) for m1, m2 in
        zip(structure_wrangler.supercell_matrices, sw.supercell_matrices)
    )
