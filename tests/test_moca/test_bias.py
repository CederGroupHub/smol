...
def test_compute_bias_change(mcbias):
    ...
    occu = gen_random_occupancy(
        mcbias.sublattices, mcbias.inactive_sublattices)
    new_occu = occu.copy()
    for _ in range(50):
        ...
        step.append((i, sp))
        if i == 81: raise(ValueError, "81!!!!")
        new_occu[i] = sp
    print(mcbias.sublattices)
    print(step)
    assert (mcbias.compute_bias_change(occu, step) ==
            pytest.approx(
...
def test_mcbias_factory(all_sublattices):
    for bias in bias_classes:
        assert isinstance(
            mcbias_factory(bias.__name__, *all_sublattices), bias)
...
def test_bad_fugacity_fractions(fugacity_bias):
    ...
    with pytest.raises(ValueError):
        fug_fracs[0]['foo'] = 0.4
        FugacityBias(
            fugacity_bias.sublattices, fugacity_bias.inactive_sublattices,
